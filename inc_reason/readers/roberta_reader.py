# Code based from https://github.com/alontalmor/LeapOfThought/blob/master/LeapOfThought/allennlp_models/dataset_readers/transformer_binary_qa_reader.py
...
class TransformerBinaryReader(DatasetReader):
    ...
    def text_to_instance(self,  # type: ignore
        ...
        ## Property name for debug
        __prop = org_metadata.get("property",{})
        property_name = __prop.get("predicate","None")
    ...
    def transformer_features_from_qa(self, question: str, context: str):
        ...
        if context is not None and len(context) > 0:
            ...
            tokensa = self._tokenizer.tokenize(question)
            tokensb = self._tokenizer.tokenize(context)
            self._tokenizer._add_special_tokens = True
            tokens = self._tokenizer.add_special_tokens(tokensa,tokensb)
        else:
